...
/*   Updated: 2019/11/25 11:07:17 by adtheus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_display_str(t_struc *su, char *str)
{
	...
	{
		length = (su->precision != -1) ? su->precision : ft_strlen1(str);
...
void	ft_display_nb(t_struc *su, char *str)
{
	...
	else
	{
		if (*str == '-')
			su->precision++;
		su->precision = su->precision > length ? su->precision - length : 0;
		while (su->field_wid-- > length + su->precision && ++su->ret)
			write(1, " ", 1);
		if (*str == '-' && ++su->ret)
			write(1, str++, 1);
		while (su->precision-- && ++su->ret)
			write(1, "0", 1);
		while (*str && ++su->ret)
			write(1, str++, 1);
	}
...
void	ft_display_ptr(t_struc *su, char *str)
{
	...
	*str == '0' && (su->precision == 0 || su->precision == -1) ? *str = 0 : 0;
	su->field_wid -= 2;
	length = ft_strlen1(str);
	if (su->minus)
	{
	...
	{
		su->precision = su->precision > length ? su->precision - length : 0;
		while (su->field_wid-- > su->precision + length && ++su->ret)
			write(1, " ", 1);
		write(1, "0x", 2);
		su->ret += 2;
		if (su->precision > 0)
